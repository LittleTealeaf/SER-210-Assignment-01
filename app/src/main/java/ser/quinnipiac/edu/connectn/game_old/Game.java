...
public class Game implements IGame {
    ...
    private final IComputer ai;
    private final Random random;
    private final Collection<GameListener> listeners;


    public Game() {
        this(6, 6, 4, Computer.HARD);
    }

    public Game(int columns, int rows, int connectLength) {
        this(columns, rows, connectLength, Computer.HARD);
    }

    public Game(int columns, int rows, int connectLength, IComputer ai) {
        this.ai = ai;
    ...
    public void saveToBundle(Bundle bundle) {
        IGame.super.saveToBundle(bundle);
        ai.saveToBundle(bundle);
        int columns = getColumnCount();
        int[] gameBoard = new int[getRowCount() * columns];
        for(int i = 0; i < gameBoard.length; i++) {
            gameBoard[i] = board[i/columns][i%columns];
        }
        bundle.putIntArray(BOARD,gameBoard);
    }
}
